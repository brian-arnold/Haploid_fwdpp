...
//
// For structured deme, whole individuals only migrate before ngenMig, includes haploid vector
//
template <typename structpoptype,
typename haploid_fitness_function, typename mutation_model,
typename recombination_policy,
                             ...
                             const mutation_removal_policy mp = mutation_removal_policy())
{
    ...
    // Migration and build pop-specific lookup tables:
    if( gen < ngenMig){
        // migration still occurring
        auto lookups = migrate_and_calc_fitness(r, pop, ff, N1, N2, m12, m21);
    }else{
        // all migration halted
        auto lookups = migrate_and_calc_fitness(r, pop, ff, N1, N2, 0.0, 0.0);
    }
...
//
// For structured deme, migration only from interpopulation recombination after ngenMig
//
template <typename structpoptype,
typename haploid_fitness_function, typename mutation_model,
typename recombination_policy,
...
//
// For structured deme, migration only from interpopulation recombination before ngenMig
//
template <typename structpoptype,
typename haploid_fitness_function, typename mutation_model,
typename recombination_policy,
